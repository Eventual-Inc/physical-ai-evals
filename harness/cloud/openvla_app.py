...
Run::
    ...
    modal run harness/cloud/openvla_app.py --suites libero_spatial --episodes 5 --seed 7
    modal run harness/cloud/openvla_app.py --download-only
"""
...
def _download(model_id: str, model_revision: str = "") -> dict:
    from harness.cloud.modal_infra import resolve_hf_model_path
    from harness.cloud.sweep import OPENVLA_REVISIONS, immutable_model_id
...
def modal_main(
    ...
    if policy_type != "openvla":
        raise SystemExit(
            f"policy_type={policy_type!r} is in harness/cloud/vla_jepa_app.py. "
