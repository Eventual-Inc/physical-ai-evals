...
def _build_policy(
    ...
    load_path = model_id
    if model_revision:
        from harness.cloud.modal_infra import MODEL_CACHE_DIR, resolve_hf_model_path
...
class LiberoRollout:
    ...
    def _env(self, suite: str, task_id: int, seed: int):
        ...
        if key not in self._env_cache:
            from harness.bench.libero import make_env
    ...
    def rollout(self, suites: Series, task_ids: Series, init_state_ids: Series, seeds: Series) -> list[dict]:
        from harness.bench.libero import libero_init_states, run_episode
...
def build_rollout_dataframe(
    ...
    **runner_kwargs,
    ...
    if policy_type == "openvla":
        from harness.cloud.sweep import resolve_openvla_config
        ...
        from harness.cloud.sweep import resolve_vla_jepa_config
